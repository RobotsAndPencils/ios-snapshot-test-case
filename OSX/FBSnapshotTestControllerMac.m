...
- (BOOL)saveFailedReferenceImage:(NSImage *)referenceImage
                      ...
                      identifier:(NSString *)identifier
                           error:(NSError **)errorPtr
{
  NSData *referencePNGData = [referenceImage PNGRepresentation];
  NSData *testPNGData = [testImage PNGRepresentation];
  ...
  NSData *diffImageData = [diffImage PNGRepresentation];
...
- (BOOL)_performPixelComparisonWithViewOrLayer:(NSView *)viewOrLayer
                                      selector:(SEL)selector
                                    identifier:(NSString *)identifier
                                         error:(NSError **)errorPtr
{
  ...
  if (nil != referenceImage) {
    NSImage *snapshot = [self _snapshotViewOrLayer:viewOrLayer];
...
- (NSImage *)_snapshotViewOrLayer:(id)viewOrLayer
{
  CALayer *layer = nil;
  ...
  if ([viewOrLayer isKindOfClass:[NSView class]]) {
    ...
    [NSException raise:@"Only NSView and CALayer classes can be snapshotted" format:@"%@", viewOrLayer];
  }
  return nil;
}

- (NSImage *)_renderLayer:(CALayer *)layer
{
...
- (NSImage *)_renderView:(NSView *)view
{
  NSSize viewSize = view.bounds.size;
  NSSize imageSize = NSMakeSize( viewSize.width, viewSize.height );
